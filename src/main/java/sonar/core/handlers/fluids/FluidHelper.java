...
public class FluidHelper extends FluidWrapper {
	...
	@Override
	public StoredFluidStack removeFluids(StoredFluidStack remove, BlockEntity tile, Direction face, ActionType type, ITankFilter filter) {
	...
	@Override
	public StoredFluidStack addFluids(StoredFluidStack add, BlockEntity tile, Direction face, ActionType type, ITankFilter filter) {
	...
	@Override
	public void transferFluids(BlockEntity from, BlockEntity to, Direction dirFrom, Direction dirTo, ITankFilter filter) {
	...
	public static StoredFluidStack addStack(StoredFluidStack add, IFluidHandler handler, Direction dir, ActionType action) {
		add.stored -= handler.fill(add.getFullStack(), !action.shouldSimulate());
		return add;
	}

	public static StoredFluidStack removeStack(StoredFluidStack remove, IFluidHandler handler, Direction dir, ActionType action) {
		FluidStack drained = null;
		remove.stored -= (drained = handler.drain(remove.getFullStack(), !action.shouldSimulate())) != null ? drained.getAmount() : 0;
		return remove;
	}

	public static StorageSize getFluids(List<StoredFluidStack> fluids, IFluidHandler handler, Direction dir) {
		long stored = 0;
		long maxStorage = 0;
